...
const _UserDetailScreen: React.FC<UserDetailScreenProps> = ({
  userReducer,
  onUserFollow,
  onUserUnfollow,
  onGetUserFollowers,
  ...
  const getUserProfile = async () => {
    await axios
      .get<Response & UserModel>(`${BASE_URL}users/${user_id}`)
      .then(response => {
    ...
    await axios
      .get<[PostModel]>(`${BASE_URL}posts/user/${user_id}`)
      .then(response => {
        if (response.data) {
    ...
    await axios
      .get<Response & AccountModel>(`${BASE_URL}accounts/${acc_id}`)
      .then(response => {
        if (response.data.response) {
  ...
  const check = async () => {
    await axios
      .post(`${BASE_URL}userfollowers/checkIfFollows`, {
        follower_id: user.id,
    ...
    <View style={styles.container}>
      ...
      <View style={styles.user_info_container}>
        ...
        <View style={{flex: 1}}>
          ...
          <Text style={styles.user_account_name_text}>
            @{profileAccount?.name}
          </Text>
        </View>
      </View>
      ...
      <View style={styles.followings_detail_container}>
        <View>
          <TouchableOpacity
            onPress={() =>
              navigation.navigate('FollowerListPage', {
                title: 'Followers',
                user_id: user_id,
              })
            }>
...
const UserDetailScreen = connect(mapToStateProps, {
  onGetUsersPosts,
  onGetUserFollowers,
  onUserFollow,
  onUserUnfollow,
